...
from skimage.feature import hog, match_descriptors, corner_harris, corner_peaks, ORB, plot_matches
...
class ImageDescriptors(object):
    ...
    def generate_descriptor(image, method=1, cellSize=(10, 10)):
        ...
        elif method == ImageDescriptors.ORB:
            return ImageDescriptors.ORB(image)

        elif method == ImageDescriptors.BRIEF:
            return ImageDescriptors.BRIEF(image)
    ...
    def ORB(img):
        ...
        for y in range(0, imgheight, M):
            for x in range(0, imgwidth, N):
                imZeros = np.zeros((imgwidth, imgheight))
                y1 = y + M
                x1 = x + N
                tiles = img[y:y + M, x:x + N]
                imZeros[y:y + M, x:x + N] = tiles
                try:
                    kp= orb.detect(imZeros, None)
                except:
                    plt.imshow(imZeros)
                    plt.show()
                    print("hola")
                kp , des = orb.compute(imZeros, kp)
                ders.append(des)
                img2 = cv2.drawKeypoints(tiles, kp, None, color=(0, 255, 0), flags=0)
